...
const ROLES = [
  ...
  { value: 'TEACHER', label: 'Teacher', desc: 'Create courses and guide students' },
  { value: 'ADMIN', label: 'Admin', desc: 'Manage courses and students' },
];

export function RegisterPage() {
  const { register } = useAuth();
  const navigate = useNavigate();
  const [form, setForm] = useState({ name: '', email: '', age: '', password: '', confirmPassword: '', role: 'STUDENT' });
  ...
  const validate = () => {
    ...
    if (form.role === 'STUDENT') {
      ...
      else if (!Number.isFinite(Number(form.age)) || Number(form.age) <= 0) e.age = 'Enter a valid age';
    }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {form.role === 'STUDENT' && (
        <div>
          <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--text-main)' }}>Age</label>
          <input type="number" min="1" value={form.age} onChange={(e) => setForm((f) => ({ ...f, age: e.target.value }))}
            placeholder="18" className={cn('input-field', errors.age && 'border-rose-400')} />
