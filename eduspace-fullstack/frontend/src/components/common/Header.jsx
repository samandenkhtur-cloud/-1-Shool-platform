      ...
      <div className="flex items-center gap-2 ml-auto">
        ...
        <div ref={notifRef} className="relative">
          ...
          >
            ...
            {unread > 0 && (
              <span className="absolute top-1.5 right-1.5 min-w-[14px] h-[14px] bg-red-500 rounded-full text-[9px] font-bold text-white flex items-center justify-center px-0.5 border-2 border-white dark:border-dark-card">
                {unread > 9 ? '9+' : unread}
              </span>
            )}
          </button>
          ...
          {showNotifs && (
            ...
            >
              <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid var(--border-col)' }}>
                ...
                <button onClick={() => markAll.mutate()} className="text-xs font-semibold text-brand-600 dark:text-brand-400 hover:text-brand-700">
                  {t.markAllRead}
                </button>
              </div>
                ...
                {notifications.map((n) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold" style={{ color: 'var(--text-main)' }}>{n.title}</p>
                      <p className="text-xs mt-0.5 line-clamp-2" style={{ color: 'var(--text-muted)' }}>{n.message}</p>
                      <p className="text-[10px] mt-1" style={{ color: 'var(--text-muted)', opacity: 0.6 }}>{n.time}</p>
                    </div>
                    {!n.read && <span className="w-1.5 h-1.5 bg-brand-500 rounded-full flex-shrink-0 mt-1.5" />}
                  </div>
                ))}
              </div>
            </div>
